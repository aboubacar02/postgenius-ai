    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 backdrop-blur-md shadow-2xl">
      <div className="relative mx-auto aspect-[9/16] w-full max-w-[280px] overflow-hidden rounded-2xl border border-white/10 bg-zinc-950">
        ...
        <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
          <span className="rounded-full bg-black/60 px-2.5 py-1 font-mono text-[11px] font-bold text-white backdrop-blur-sm">
            {String(activeIdx + 1).padStart(2, '0')}/{sceneCount}
          </span>
          <span className="rounded-full bg-violet-600/80 px-2.5 py-1 font-mono text-[11px] font-bold text-white backdrop-blur-sm">
            {sceneDur}s
          </span>
        </div>
        ...
        {hasVideo && (
          <div className="absolute top-3 right-3 z-10">
            <span className="flex items-center gap-1 rounded-full bg-green-500/80 px-2 py-0.5 font-mono text-[10px] font-bold text-white backdrop-blur-sm">
        ...
        <div className="absolute bottom-3 left-0 right-0 z-10 flex items-center justify-center gap-4">
          <button onClick={() => jumpTo(Math.max(0, activeIdx - 1))} disabled={activeIdx === 0} className="flex min-h-[44px] min-w-[44px] items-center justify-center text-white/60 hover:text-white disabled:opacity-30 text-xs transition-colors">
            &#9198;
          </button>
          <button onClick={togglePlay} className="flex size-12 items-center justify-center rounded-full bg-violet-600 text-white shadow-lg shadow-violet-500/30 hover:scale-105 transition-transform">
            {playing ? <Pause className="size-5" /> : <Play className="size-5 ml-0.5" />}
          </button>
          <button onClick={() => jumpTo(Math.min(sceneCount - 1, activeIdx + 1))} disabled={activeIdx >= sceneCount - 1} className="flex min-h-[44px] min-w-[44px] items-center justify-center text-white/60 hover:text-white disabled:opacity-30 text-xs transition-colors">
            &#9197;
          </button>
        </div>
      </div>
